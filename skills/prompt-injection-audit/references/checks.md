...
- *Trust source (bad):* identity taken from the request body, query string, a header the client sets freely, or (worst of all) from the natural-language prompt itself (`user_id` parsed out of the message, "the user says they are …").
...
**Severity.** Usually **High to Critical** when cross-user data is reachable. **OWASP:** LLM01 (Prompt Injection), LLM06 (Excessive Agency).
...
**Severity.** **High to Critical** for broad credentials or cross-user reads. **OWASP:** LLM02 (Sensitive Information Disclosure), LLM01.
...
**Severity.** Usually **Medium** on its own (**Medium to High** if it can leak secrets or PII). **OWASP:** LLM05 (Improper Output Handling), LLM07 (System Prompt Leakage), LLM02.
...
**Severity.** **High to Critical**, highest for destructive or financial actions. **OWASP:** LLM06 (Excessive Agency).
...
**Severity.** **Medium to High**, higher if retrieval is cross-user. **OWASP:** LLM01 (Indirect Prompt Injection); LLM08 (Vector and Embedding Weaknesses) when a vector store is involved.

---

### G3: Second-order (stored) injection

**The failure.** Attacker text stored now is executed later, often in someone else's session. A malicious "bio," support-ticket body, product review, or filename is saved today, then later loaded into a prompt (a nightly summary, an admin dashboard, another user's feed) where its instructions run with that later context's privileges.
...
**Severity.** **Low to Medium** (higher if it leaks secrets or detailed schema). **OWASP:** LLM02 (Sensitive Information Disclosure), LLM07.
...
**FAIL looks like.**
- The model path (inputs and especially tool/action calls) is not logged in any durable, reviewable way, so injection attempts would leave no trace. Cite the handler/tool code where logging is absent. Grade this **Low to Medium**. It is an enabling weakness, not a breach.
...
**Severity.** **Low to Medium** (enabling weakness). **OWASP:** no clean LLM-Top-10 ID; supports detection/response for LLM01. Report the mapping honestly as "monitoring gap; no direct OWASP-LLM ID."
